...
function positionChildVertical(position, constraint, blockSize, childBlockSize) {
  if (position.top < 0 && position.bottom >= 0) {
...
registerLayout('relative', class {
  ...
  *layout(children, edges, constraints, styleMap) {
    const relativeConstraints = parseRelativeConstraints(styleMap.get('--relative-constraints').toString(), constraints.fixedInlineSize);
    ...
    const childFragmentMap = {};
		for (let childName of sortedChildNamesVertical) {
			applyVerticalRules(relativeConstraints, childPositions, childName, constraints.fixedBlockSize);
      const fragment = yield* measureChild(childrenMap[childName], childPositions[childName]);
      childFragmentMap[childName] = fragment;
      positionChildVertical(childPositions[childName], relativeConstraints[childName], constraints.fixedBlockSize, fragment.blockSize);
		}
