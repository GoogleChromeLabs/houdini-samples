...
registerAnimator('hidey-bar', class TwitterHeader {
  ...
  animate(currentTime, effect) {
    ...
    // Attach to document timeline so we keep animating event when scrolling
    // stops.
    if (isScrolling)
      this.attachDocumentTimeline();
  ...
  attachDocumentTimeline() {
    if (this.attachedTime)
      return;

    this.options.documentTimeline.attach(this);
    this.attachedTime = true;
    this.frameCount = 0;
  }

  detachDocumentTimeline() {
    if (!this.attachedTime)
      return;
